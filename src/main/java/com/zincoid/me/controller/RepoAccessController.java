...
public class RepoAccessController {
    ...
    public ApiResponse<List<RepoAccess>> sentPending() {
        return ApiResponse.success(repoAccessService.sentPending(AuthCtx.getUserId()));
    }

    @GetMapping("/access/sent/resolved")
    public ApiResponse<List<RepoAccess>> sentResolved() {
        return ApiResponse.success(repoAccessService.sentResolved(AuthCtx.getUserId()));
    }

    @GetMapping("/access/received/pending")
    public ApiResponse<List<RepoAccess>> receivedPending() {
        return ApiResponse.success(repoAccessService.receivedPending(AuthCtx.getUserId()));
    }

    @GetMapping("/access/received/resolved")
    public ApiResponse<List<RepoAccess>> receivedResolved() {
        return ApiResponse.success(repoAccessService.receivedResolved(AuthCtx.getUserId()));
    }
}
